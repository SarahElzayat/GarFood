...
void main(){
    ...
    // convert from texture coordinate space ranges (from 0 to 1) to NDC space ranges (from -1 to 1)
    vec2 ndc_coord = tex_coord * 2.0 - 1.0; 
    // the squared length of the 2D pixel location the NDC space
    float squared_length = pow(length(ndc_coord), 2.0); 
    //apply vignette, divide the scene color
    // by 1 + the squared length of the 2D pixel location the NDC space
    frag_color /= (1.0 + squared_length);  
}