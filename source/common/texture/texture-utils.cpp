...
our::Texture2D *our::texture_utils::loadImage(const std::string &filename, bool generate_mipmap)
{
    ...
    texture->bind(); // binding the generated texture to GL_TEXTURE_2D
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, size.x, size.y, 0, GL_RGBA, GL_UNSIGNED_BYTE, pixels); // specify a two-dimensional texture image
    // GL_TEXTURE_2D for the target
    // 0 --> level
    // GL_RGBA8 --> internal format, number of color components in the texture, 8 bits are enough for the current usage
    // size.x, size.y --> width and height of the texture
    // 0 --> border, must be 0 for backward compatibility
    // GL_RGBA --> format of the pixel data
    // GL_UNSIGNED_BYTE --> data type of the pixel data
    // pixels --> pointer to the image data in memory

    if (generate_mipmap)
        glGenerateMipmap(GL_TEXTURE_2D); // Generate mipmaps for the target texture
